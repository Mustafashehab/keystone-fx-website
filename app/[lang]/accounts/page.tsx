...
export default async function Accounts({ params }: { params: Promise<{ lang: Lang }> }) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-slate-900 mb-4">{t(lang, "accounts.features.title")}</h2>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto">{t(lang, "accounts.features.subtitle")}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="group relative rounded-2xl border-2 border-slate-200 bg-white p-8 transition-all hover:border-yellow-400 hover:shadow-2xl hover:-translate-y-2">
            <div className="mb-6 inline-block rounded-full bg-yellow-400 p-4 text-4xl shadow-lg group-hover:scale-110 transition-transform">🚀</div>
            <h3 className="text-2xl font-bold text-slate-900 mb-3">{t(lang, "accounts.features.global.title")}</h3>
            <p className="text-slate-600 leading-relaxed">{t(lang, "accounts.features.global.description")}</p>
          </div>
          <div className="group relative rounded-2xl border-2 border-slate-200 bg-white p-8 transition-all hover:border-yellow-400 hover:shadow-2xl hover:-translate-y-2">
            <div className="mb-6 inline-block rounded-full bg-yellow-400 p-4 text-4xl shadow-lg group-hover:scale-110 transition-transform">🔒</div>
            <h3 className="text-2xl font-bold text-slate-900 mb-3">{t(lang, "accounts.features.secure.title")}</h3>
            <p className="text-slate-600 leading-relaxed">{t(lang, "accounts.features.secure.description")}</p>
          </div>
          <div className="group relative rounded-2xl border-2 border-slate-200 bg-white p-8 transition-all hover:border-yellow-400 hover:shadow-2xl hover:-translate-y-2">
            <div className="mb-6 inline-block rounded-full bg-yellow-400 p-4 text-4xl shadow-lg group-hover:scale-110 transition-transform">⚡</div>
            <h3 className="text-2xl font-bold text-slate-900 mb-3">{t(lang, "accounts.features.fast.title")}</h3>
            <p className="text-slate-600 leading-relaxed">{t(lang, "accounts.features.fast.description")}</p>
          </div>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 to-slate-800 py-24">
        <div className="absolute inset-0 opacity-5">
          <div className="absolute inset-0" style={{ backgroundImage: `linear-gradient(45deg, #fbbf24 25%, transparent 25%, transparent 75%, #fbbf24 75%, #fbbf24), linear-gradient(45deg, #fbbf24 25%, transparent 25%, transparent 75%, #fbbf24 75%, #fbbf24)`, backgroundSize: '60px 60px', backgroundPosition: '0 0, 30px 30px' }}></div>
        </div>
        <div className="relative mx-auto max-w-6xl px-6">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-white mb-4">{t(lang, "accounts.steps.title")}</h2>
      ...
      <section className="bg-slate-900 text-white py-24">
        <div className="mx-auto max-w-6xl px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-slate-300 max-w-2xl mx-auto">{t(lang, "accounts.compare.subtitle")}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {accountTypes.map((account, idx) => (
              <div key={idx} className={`group relative rounded-3xl p-8 transition-all hover:-translate-y-2 ${idx === 1 ? 'bg-gradient-to-br from-yellow-400 to-yellow-500 text-slate-900 transform md:scale-105 shadow-2xl shadow-yellow-400/50 ring-4 ring-yellow-300' : 'bg-slate-800 border-2 border-slate-700 hover:border-yellow-400'}`}>
                {idx === 1 && (
                ...
                <h3 className={`text-3xl font-bold mb-8 ${idx === 1 ? 'text-slate-900' : 'text-white'}`}>{account.name}</h3>
                <div className="space-y-6 mb-8">
                  <div className={`text-sm font-semibold ${idx === 1 ? 'text-slate-700' : 'text-slate-400'}`}>{t(lang, "accounts.compare.minDeposit")}</div>
                  <div className="text-5xl font-bold">{account.minDeposit}</div>
                  <div className="pt-4 border-t border-slate-700">
                    <div className={`text-sm font-semibold mb-2 ${idx === 1 ? 'text-slate-700' : 'text-slate-400'}`}>{t(lang, "accounts.compare.leverage")}</div>
                ...
                <div className="space-y-3 mb-8">
                  {account.features.map((feature, fIdx) => (
                    <div key={fIdx} className="flex items-start gap-3">
                      ...
                      <span className={idx === 1 ? 'text-slate-800' : 'text-slate-300'}>{feature}</span>
                    </div>
                  ))}
                </div>
